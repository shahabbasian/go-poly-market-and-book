...
// UpsertMarket inserts or updates a market row by slug.
// Slug is the single unique identifier for a market, so this method avoids
// duplicate rows even when token_id_yes is present or changes over time.
